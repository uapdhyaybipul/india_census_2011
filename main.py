...
if plot:
    ...
    if state == 'Overall India':
        fig= scatter_mapbox(df, lat='Latitude', lon='Longitude', zoom=3, hover_name='District', size=primary_selection, color=secondary_selection, mapbox_style="open-street-map")
        st.plotly_chart(fig)
    else:
        state_df= df[df['State']== state]

        fig = scatter_mapbox(state_df, lat='Latitude', lon='Longitude', zoom=6, hover_name='District',
